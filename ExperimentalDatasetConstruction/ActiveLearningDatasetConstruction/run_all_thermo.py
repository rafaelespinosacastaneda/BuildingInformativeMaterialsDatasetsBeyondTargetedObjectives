...
def return_model(modelname,random_state):
    ...
    if modelname == 'xgb':
        ...
        num_parallel_tree=4 ,device="cpu",random_state=random_state
        )
...
# Example usage inside run_all_thermo.py:
...
if growingCriteria == 'grow_random':
    ...
    grow_2QBC(model['rf'], model['xgb'],
             ...
             xTest, yTest,
             target1=target1,
             target2=target2,           # <-- NEW: second disagreement target (optional)
    ...
    grow_3QBC(model['rf'], model['xgb'],
             ...
             target1=target1,
             target2=target2,
             target3=target3,              # <-- NEW: third disagreement target (optional)
